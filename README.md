...
With Swift Package Manager: In Xcode, **File ▸ Add Package Dependencies…** and paste the URL:
...
- **Tap a hashtag to select every copy of it.** Capitalization doesn't matter, so `#Sun` and `#sun` count as one tag.
- **Your text's formatting is kept.** Fonts, colors, and links stay exactly as they were through every action.
- **The text stays put** when you tap; it won't scroll to the top.
- **Clean-up is optional.** Call `cleanUpHashtags()` to remove duplicates and stray `#`s; TapTagKit never changes your text unless you ask. In SwiftUI, clean a string with `TapTextView.cleanedHashtags(in:)`.
